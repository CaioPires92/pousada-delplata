...
describe("POST /api/crm/followup/post-stay", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("requires token", async () => {
    const req = new Request("http://localhost/api/crm/followup/post-stay", { method: "POST" });
    const res = await POST(req);
    expect(res.status).toBe(401);
  });

  it("supports dryRun", async () => {
    const req = new Request("http://localhost/api/crm/followup/post-stay", {
      method: "POST",
